...
corpus_dict_text = {}  # Stores title and full text
corpus_dict_exact = {}  # Stores title & text without preprocessing (for exact matching)
...
with open(corpus_path, 'r', encoding='utf-8') as f:
    for line in f:
        ...
        try:
            ...
            exact_text = preprocess_text(f"{doc['title']} {doc['text']}")  # Preprocess like other modes
            
            corpus_dict_title[doc_id] = title  # Store only title
            corpus_dict_text[doc_id] = f"{title} {full_text}"  # Store title and full text
            corpus_dict_exact[doc_id] = exact_text  # Store exact (unprocessed)
...
# Get unique queries
unique_queries = df.groupby("query_id")["corpus_id"].first().reset_index()
...
for _, row in unique_queries.iterrows():
    ...
    if corpus_id in corpus_dict_exact:
        test_queries_exact[query_id] = {
            "query": corpus_dict_exact[corpus_id],
...
vectorizer_exact = TfidfVectorizer()
...
doc_vectors_exact = vectorizer_exact.fit_transform(list(corpus_dict_exact.values()))
...
# Retrieval for exact match (title & text)
all_results_exact = []
for query_id, data in test_queries_exact.items():
    all_results_exact.extend(retrieve_and_rank(
        data["query"], query_id, data["run_name"], vectorizer_exact, doc_vectors_exact, list(corpus_dict_exact.keys())
    ))

# Retrieval for title only
all_results_title = []
for query_id, data in test_queries_title.items():
    all_results_title.extend(retrieve_and_rank(
        data["query"], query_id, data["run_name"], vectorizer_title, doc_vectors_title, list(corpus_dict_title.keys())
    ))

# Retrieval for text only
all_results_text = []
for query_id, data in test_queries_text.items():
    all_results_text.extend(retrieve_and_rank(
        data["query"], query_id, data["run_name"], vectorizer_text, doc_vectors_text, list(corpus_dict_text.keys())
    ))

# Save results to files
results_exact_path = "results/all_results_exact_match.txt"
results_title_path = "results/all_results_title.txt"
results_text_path = "results/all_results_text.txt"

with open(results_exact_path, "w", encoding="utf-8") as f:
    f.write("\n".join(all_results_exact) + "\n")
...
print(f"✅ Results saved to {results_exact_path}, {results_title_path}, and {results_text_path}")
